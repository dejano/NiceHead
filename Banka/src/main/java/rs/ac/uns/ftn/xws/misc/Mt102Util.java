...
public class Mt102Util {
	...
	/*
	 * Key = Bank Code of Banks who are going to get paid out, paymentOrders
	 * (payouts)
	 */
	public static Map<String, List<PaymentOrder>> getPaymentOrders() {
		...
		List<PaymentOrder> paymentOrders = ParserUtil
				.transformXmlListIntoPaymentOrderList(PaymentOrderDataDao.getPaymentOrders());

		for (PaymentOrder paymentOrder : paymentOrders) {
			String creditorBankCode = paymentOrder.getCreditorAccountDetails().getAccountNumber()
					.substring(0, 3);
	...
	/*
	 * This method creates MT102 based on unprocessed payment orders stored in
	 * XML DB.
	 */
	public static Mt102 createMt102(String creditorBankCode, BankDetails creditorBankDetails,
		...
		// TODO @baandjo iskoristi mapu - redundantno je - izvuci kod poziva
		// funkcije
		...
		if (paymentOrders.size() > 0) {
			...
			mt102.setDate(paymentOrders.get(0).getOrderDate()); // trebalo bi
																// new Date() ->
																// XMLCalendar

			// TODO what if now debtor has insufficient funds?
			for (PaymentOrder paymentOrder : paymentOrders) {
			...
			Mt102Ref mt102Ref = Mt102RefUtil.createMt102Ref(mt102.getPayments().getPayment(),
					messageId);
	...
	public static void main(String[] args) {
		...
		// Mt102Ref mt102ref =
		// Mt102RefUtil.createMt102Ref(mt102.getPayments().getPayment(),
		// "19191");
