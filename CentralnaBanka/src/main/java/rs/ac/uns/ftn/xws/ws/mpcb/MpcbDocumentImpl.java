...
public class MpcbDocumentImpl implements MpcbDocument {
	...
	public Mt900 rtgsRequest(Mt103 rtgsRequestPart) throws MpException {
		...
		String cert = CertMap.getCert(rtgsRequestPart);
		...
		String debtorBankSwiftCode = rtgsRequestPart.getDebtorBankDetails().getSwiftCode();
		String creditorBankSwiftCode = rtgsRequestPart.getCreditorBankDetails().getSwiftCode();

		// validate swift codes
		if (!CentralBankUtil.isSwiftCodeValid(debtorBankSwiftCode)) {
			throw new MpException("Debtor swift code invalid.", MpExceptionEnum.INVALID_SWIFT_CODE);
		}
		...
		BanksDataDao.updateBankBalance(debtorBankSwiftCode, debtorBankBalance.subtract(amount));

		// update creditor balance
		creditorBankBalance = BanksDataDao.getBankBalance(creditorBankSwiftCode);
		BanksDataDao.updateBankBalance(creditorBankSwiftCode, creditorBankBalance.add(amount));

		// send rtgs confirm message to creditor's bank
		MpbDocumentClient.invokeRtgsApproval(rtgsRequestPart);

		ret = ObjectMapper.getMt900(rtgsRequestPart);
	...
	public void clearingRequest(Mt102 clearingRequestPart) throws MpException {
		CertMap.getCert(clearingRequestPart);
		
		LOG.info("Executing operation clearingRequest");

		System.out.println("Clearing request ws called, mt102 : "
				+ clearingRequestPart.getMessageId());

		if (!XmlHelper.validate(clearingRequestPart, CentralBankConstants.XSD_PATH + "mp.xsd"))
			throw new MpException("Invalid xml.", MpExceptionEnum.INVALID_XML);

		String debtorBankSwiftCode = clearingRequestPart.getDebtorBankDetails().getSwiftCode();
		String creditorBankSwiftCode = clearingRequestPart.getCreditorBankDetails().getSwiftCode();
		...
		if (!CentralBankUtil.areAllPaymentsToSameBank(clearingRequestPart)) {
			throw new MpException("There are invalid payments.", MpExceptionEnum.MULTIPLE_BANKS);
		}

		// validate total amount
		if (!CentralBankUtil.isTotalAmountValid(clearingRequestPart)) {
			throw new MpException("Invalid total amount.", MpExceptionEnum.INVALID_AMOUNT);
		}
